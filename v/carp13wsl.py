...
model_path = "/mnt/z/nn/cap13.nn"
model_path_backup = "/mnt/z/nn/cap13b.nn"
...
class ModelClass():
  def __init__(s):
    ...
    # hyperparameters
    s.batch_size = 32 #48 #48 #16 # 32 # how many independent sequences will we process in parallel?
    ...
    s.max_iters = 2000 #1000 #1000 #20 #2000 #100 #0 #1000 #5000
    ...
    """
    # ------------
    s.n_embd = 256 #128 #32#128 #64 #256 #128 #64 #256 #vector dimensions
    s.n_head = 12 #8 #16 #12 #8 #4 #8 
    s.n_layer = 12 #8 #16 #12 #8 #4 #8
    s.dropout = 0.0
...
# hyperparameters
batch_size = 16 #256 #32 #48 #16 # 32 # how many independent sequences will we process in parallel?
block_size = 64 #128 #80 #128 #128 #32 #128 #32#128 #64 #256 #64 #32 # what is the maximum context length for predictions?  ... tokens per batch?
max_iters = 2000 #1000 #1000 #20 #2000 #100 #0 #1000 #5000
eval_interval = 20 #100
learning_rate = 1e-3
device = 'cuda' if torch.cuda.is_available() else 'cpu'
eval_iters = 100 #200 #200
"""n_embd = 64
n_head = 4
n_layer = 4
dropout = 0.0
...
"""
# ------------
n_embd = 64 #256 #128 #128 #32#128 #64 #256 #128 #64 #256 #vector dimensions
n_head = 8 #16 #12 #8 #4 #8 
n_layer = 8 #16 #16 #16 #12 #8 #4 #8
dropout = 0.0
...
#path="z:\\corpus2"
path="z:\\Gore-Vidal_-_Sytvorenieto_-_987-b.txt"

path = "/mnt/z/Corpus/" #Z:\\corpus"
...
def uni():
    ...
    for (root, dirs, file) in os.walk(path):
        for f in file:
          ...
          #with open(root+"\\"+f, 'rt') as f: #, encoding="utf8") as f:
          with open(root+"/"+f, 'rb') as f: #, encoding="utf8") as f: 
            #text = text + f.read().encode(encoding='UTF-8', errors='ignore').decode('cp1251', errors='ignore')
            text = text + f.read().decode('cp1251', errors='ignore')
            #text = text) + f.read().encode(encoding='UTF-8', errors='ignore').decode('utf-8')
            print("\n"+text[0:200]+"\n")
            #if '.txt' in f:
            #    print(f)
def ansi():
    ...
    for (root, dirs, file) in os.walk(path):
        for f in file:
          ...
          with open(root+"/"+f, 'r', encoding="cp1251") as fh: #,
            print(root+"\\"+f)
            s = fh.read()            
            print(s[:500])
            text = text + s
...
def interact_afterload(model_path):
  ...
  while True:
    ...
    prompt_now = input("\nEnter prompt...(1 for continuing by the last half of this one - valid for 2-nd etc. prompts\n")
...
model_path = "Z:\\nn\\cap9.nn" #not in cog for the next reading
torch.save(model.state_dict(),model_path)
