...
data = torch.tensor(encode(string_tokenized), dtype=torch.long)
...
def interact_afterload(model_path):
  ...
  while True:
    ...
    print("Context decoded?:", tokenizerLOAD.decode(context))
    ...
    prompt_prev = tokenizerLOAD.decode(m.generate(context, max_new_tokens=kolko)[0].tolist())
    print(prompt_prev)
