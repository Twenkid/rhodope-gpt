...
LOAD_TO_GEN = "1"
TRAIN_NEW = "3"
load_tokeinzer = True #False #True #False # token4cog ... True #False #TRAIN NEW True #False first time for training it - training is slow, 9-10 min for 144 MB
...
#corpus = "z:/corpus/"
#corpus = "C:\Back2023\16-5-2023-gpt-new\cog"
...
##model_path = "z:\\nncog\\gpt_cog_b.nn" #"/content/cap12_256-16-16-block128-batch-32_Vidal.nn"
##model_path_backup = "z:\\nncog\\gpt_cog_back.nn" #content/gpt_b.nn"

model_path_backup = "z:\\nncogcode\\gpt_cogcode_back.nn" #content/gpt_b.nn"
model_path = model_path_backup

m_vocab_size = 4000  #1000 --> 13M
vocab_size = m_vocab_size + 5 # 50250 + 5 #BPE ... //not needed for cog but whatever
# ------------
#80% RAM GPU, vocab 4000
n_embd = 256 # 384 256 #128 #128 #32#128 #64 #256 #128 #64 #256 #vector dimensions
n_head = 16 #16 #16 #12 #8 #4 #8 
n_layer = 16 #16 #16 #16 #12 #8 #4 #8
dropout = 0.0
batch_size = 24 #16 #32 #256 #32 #48 #16 # 32 # how many independent sequences will we process in parallel?
...
tokenizer_path = "z:\\cog_tokenized_data\\" #/content/tokenized_data/" # no /
...
paths = [str(x) for x in Path(corpus).glob("*.py")]  #Or edit accordingly to
print(paths)
...
#*.py 
paths = [str(x) for x in Path(corpus).glob("*.*")]  #In one file for all, otherwise errors with the shapes?
...
if not load_tokeinzer:
  ...
  save_path = tokenizer_path # '/content/tokenized_data/' #21-5-2023
...
#АБФВЕФ(кдоф*(4фд+33дфсд)/4рфсд(фсдфдсф3р№)деф-34йффсд+SQRT(kdfdf(4343fdf/fdssdfkse)))
"""
...
def interact_afterload(model_path):
  ...
  while True:
    ...
    lp = 0
    while(lp == 0):
      prompt_now = input("\nEnter prompt...\n1...continuing by the last half of this one - valid for 2-nd etc. prompts; \2...Q,q for Quit, \n2...input of the output length\n")      
      lp = len(prompt_now)
      if lp==0: print("No empty prompts! Say something! :) Q,q for Quit"); continue
      if prompt_now=="2": # or prompt_now=="2":
    ...
    if n>0:
      if prompt_now == "1":
        print("1: continue previous prompt...")
        L = len(prompt_prev)
